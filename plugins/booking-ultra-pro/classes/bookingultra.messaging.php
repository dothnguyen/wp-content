...
class BookingUltraProMessaging extends BookingUltraCommon 
{
	...
	public function  send_payment_confirmed_bank($staff_member, $client, $service, $appointment, $order )
	{
		...
		$appointment_cancel_url =$bookingultrapro->appointment->get_cancel_link_of_appointment($appointment->booking_key);
	...
	public function  send_reschedule_notification_on_admin($staff_member, $client, $service, $appointment,  $bup_notify_client_reschedule )
	{
		...
		//get templates	
		$template_admin = stripslashes($this->get_option('email_reschedule'));
		$template_client =stripslashes($this->get_option('email_reschedule'));
		$template_staff =stripslashes($this->get_option('email_reschedule'));
		...
		$subject_staff = $this->get_option('email_reschedule_subject');
		$subject_admin = $this->get_option('email_reschedule_subject');
